...
const experiences = [
  {
    id: 'citic',
    company: '中信证券股份有限公司',
    role: '投资银行部实习生 (人才发展中心)',
    date: '2026.04 - 2026.06',
    tags: '股权融资 / 并购重组 / 债券发行 / 尽职调查',
    logo: 'https://logo.clearbit.com/citics.com',
    categories: [
      {
        id: 'citic-equity',
        title: '股权融资及并购',
        details: [
          {
            title: '财务尽职调查',
            desc: '负责自然人及关联方银行流水与函证核查，对<strong class="text-ink font-semibold">海量交易数据进行穿透式分析与异常标注</strong>，确保资金往来真实性；同时执行<strong class="text-ink font-semibold">销售穿行测试</strong>，核验合同、物流、回款全过程凭证逻辑，系统化管理函证收发并维护控制表，通过<strong class="text-ink font-semibold">截止性测试识别跨期风险</strong>，保障财务数据在正确期间确认。'
          },
          {
            title: 'Pitch Book与申报材料',
            desc: '为高校合作及高端制造、生物医药、航空航天等企业制作<strong class="text-ink font-semibold">Pitch Book与服务方案</strong>，梳理案例与合作历史，从竞争格局、技术壁垒、业务及财务表现等维度挖掘亮点；撰写<strong class="text-ink font-semibold">企业再融资会计专项核查报告</strong>，阐明审核要点与财务合规性；并根据发行需求制作<strong class="text-ink font-semibold">发行方案PPT及董事会议案文件</strong>，为项目推进与客户沟通提供核心支持。'
          },
          {
            title: '顾问材料撰写',
            desc: '协助完成<strong class="text-ink font-semibold">H2A项目建议书框架</strong>，对比直接A股IPO与换股吸收合并的差异；针对<strong class="text-ink font-semibold">央企并购</strong>中的保证金、集团决策等特殊审核要求，制作专项案例研究文档；研究客户公司基本面，撰写公司介绍、公告及<strong class="text-ink font-semibold">ESG内容PPT</strong>；维护并优化部门内部IPO审核动态、医疗并购、机器人行业承做案例等数据库，<strong class="text-ink font-semibold">独立建立数据底稿并沉淀可复用SOP</strong>。'
          },
          {
            title: 'ECM发行承销',
            desc: '参与某仪器技术公司<strong class="text-ink font-semibold">向特定对象发行股票项目</strong>，依照法规要求独立更新并撰写<strong class="text-ink font-semibold">认购邀请书、初步获配通知书及发行方案</strong>等核心文件，确保发行流程合规推进。'
          }
        ]
      },
      {
        ...
        details: [
          {
            title: '募集说明书与申报文件',
            desc: '协助某集团<strong class="text-ink font-semibold">DFI资质预审文件编撰及募集说明书修订</strong>、协助起草私募公司债发行登记请示、参与某科技集团<strong class="text-ink font-semibold">首次债券发行可行性研究报告</strong>、募集说明书及年度报告编制，确保披露信息准确合规。'
          },
          {
            title: 'ABS及公募REITs项目',
            desc: '深度参与某商业不动产<strong class="text-ink font-semibold">公募REITs立项</strong>工作，撰写立项报告文档，协助搭建<strong class="text-ink font-semibold">重组方案框架PPT</strong>，并<strong class="text-ink font-semibold">绘制图示底稿</strong>，展示资产重组、SPV设股、项目公司股权转让等关键环节，为项目申报及内部立项评审提供核心材料。另外，跟踪持有型不动产ABS市场动态，定期整理项目名录并撰写<strong class="text-ink font-semibold">监管审核动态市场周报</strong>。'
          },
          {
            title: '内核与投标响应',
            desc: '配合完成债券项目<strong class="text-ink font-semibold">内核回复表格的逐项复核与数据更新</strong>，形成内核回复底稿；撰写某国有资本运营公司及某投资控股公司<strong class="text-ink font-semibold">债券主承销商投标文件及报名材料</strong>，包括信息更新、项目经验等，助力团队<strong class="text-ink font-semibold">成功入库</strong>。'
          },
          {
            title: '年报及审计报告核查',
            desc: '协助核查某集团<strong class="text-ink font-semibold">2024年及2025年年度报告</strong>，比对审计报告原始稿与上传稿差异，撰写<strong class="text-ink font-semibold">年报检查要点文档</strong>，确保财务数据披露一致性；参与某科技集团2025年<strong class="text-ink font-semibold">审计报告复核</strong>。'
          }
        ]
      }
    ],
...
export default function Biz() {
  const [expandedId, setExpandedId] = useState<string | null>('citic');
  const [expandedSubId, setExpandedSubId] = useState<string | null>('citic-equity');
                      ...
                      >
                        <div className="pb-10 pl-6 md:pl-10 border-l border-dashed border-ink/20 ml-2 md:ml-4 relative mt-4">
                          ...
                          {exp.categories && exp.categories.length > 0 ? (
                            <div className="space-y-4 mb-10 w-full max-w-4xl pr-0 md:pr-4">
                              {exp.categories.map((cat: any) => (
                                <div key={cat.id} className="border border-line/30 rounded-lg overflow-hidden bg-base/40">
                                  <button
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      setExpandedSubId(expandedSubId === cat.id ? null : cat.id);
                                    }}
                                    className="w-full px-5 py-4 flex items-center justify-between text-left hover-target hover:bg-white/40 transition-colors"
                                  >
                                    <span className={`font-serif text-[15px] font-bold tracking-widest ${expandedSubId === cat.id ? 'text-accent' : 'text-ink'}`}>{cat.title}</span>
                                    {expandedSubId === cat.id ? <Minus className="w-5 h-5 text-accent" /> : <Plus className="w-5 h-5 text-sub" />}
                                  </button>
                                  <AnimatePresence>
                                    {expandedSubId === cat.id && (
                                      <motion.div
                                        initial={{ height: 0, opacity: 0 }}
                                        animate={{ height: 'auto', opacity: 1 }}
                                        exit={{ height: 0, opacity: 0 }}
                                      ...
                                      >
                                        <div className="p-6 pt-4 border-t border-line/10">
                                          <ul className="space-y-8">
                                            {cat.details.map((detail: any, i: number) => (
                                              <li key={i} className="relative group/item pl-5">
                                                <div className="absolute left-0 top-1.5 w-1.5 h-1.5 rounded-full bg-ink/30 group-hover/item:bg-accent transition-colors duration-300"></div>
                                                <div className="flex flex-col gap-2">
                                                  <div className="flex items-center gap-3">
                                                    <h4 className="text-sm font-bold text-ink tracking-widest">{detail.title}</h4>
                                                    <div className="h-[1px] flex-1 max-w-[40px] bg-ink/10"></div>
                                                  </div>
                                                  <p 
                                                    className="text-ink/80 text-sm leading-relaxed text-justify"
